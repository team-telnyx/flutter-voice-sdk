...
class NotificationService {
  ...
  static Future showNotification(RemoteMessage message) async {
    ...
    try {
      ...
      final String? telnyxCallId =
          _extractCallId(metadataMap, metadata, logger, 'showNotification');
      ...
      if (telnyxCallId == null || telnyxCallId.isEmpty) {
        logger.e(
            'NotificationService.showNotification: Missing or empty callId in metadata. Cannot show CallKit notification.'); // Corrected log
  ...
  static Future showMissedCallNotification(RemoteMessage message) async {
    ...
    try {
      ...
      final String? telnyxCallId = _extractCallId(
          metadataMap, metadata, logger, 'showMissedCallNotification');

      if (telnyxCallId == null || telnyxCallId.isEmpty) {
        logger.e(
            'NotificationService.showMissedCallNotification: Missing or empty callId in metadata. Cannot process missed call accurately.'); // Corrected log
        return;
      }
  ...
  static Future showIncomingCallUi({
    ...
    final logger = Logger()
      ..i('NotificationService.showIncomingCallUi: Displaying CallKit UI for incoming call ID: $callId');
    try {
      BackgroundDetector.ignore = true;
      final CallKitParams callKitParams = _getStandardCallKitParams(
        id: callId,
