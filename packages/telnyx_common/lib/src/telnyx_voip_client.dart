...
///
/// This class serves as the single entry point for developers using the
/// telnyx_common package. It provides a simplified, high-level API that
/// abstracts away the complexity of the underlying WebRTC implementation.
///
/// The TelnyxVoipClient is state-management agnostic, exposing all observable
/// state via Dart's native Stream<T> to allow integration with any state
/// management solution (Provider, BLoC, Riverpod, GetX, etc.).
class TelnyxVoipClient {
  /// Internal session manager for connection lifecycle.
  late final SessionManager _sessionManager;
  
  /// Internal call state controller for call management.
  late final CallStateController _callStateController;
  
  /// Internal CallKit adapter for native UI (Phase 2).
  CallKitAdapter? _callKitAdapter;
  
  /// Internal push notification gateway.
  PushNotificationGateway? _pushGateway;
  
  /// Whether the client has been disposed.
  bool _disposed = false;
  
  /// Whether native UI integration is enabled.
  ...
  void _initializeComponents() {
    ...
    _callStateController = CallStateController(_sessionManager.telnyxClient);
