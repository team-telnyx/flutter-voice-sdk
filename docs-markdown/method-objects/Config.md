...
- **`pushWhenActive`** (bool, optional): When `true`, the SDK automatically includes the configured `notificationToken` in the `answered_device_token` field of the outgoing `telnyx_rtc.answer` payload for any call answered via the normal Flutter API. This is used for "push when active" flows, where an incoming call is delivered to multiple devices via push and the backend needs to know which device answered so it can dismiss the call on the remaining devices. Defaults to `false`. When `false`, the answer payload is unchanged and existing apps see no change in behavior. When `true` and a non-empty `notificationToken` is configured, the SDK populates `answered_device_token` automatically on every accepted call. When `true` but no `notificationToken` is configured (or the token is blank), no `answered_device_token` field is sent — apps never need to send an empty value. The app does **not** need to pass an `answeredDeviceToken` argument to `acceptCall`; the SDK wires it through from the config when `pushWhenActive` is enabled. See [Push notification setup](../push-notification/app-setup.md#push-when-active-and-answered-elsewhere) for the customer-facing flow.
